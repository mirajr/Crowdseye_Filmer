...
//        NSLog(@"IS NOT RECORDING");
    ...
    if (connection == _videoConnection) {
        if (!_hasScreenshot) {
            ...
            NSString *thumbnailPath = @"thumb.jpg";
            NSString *path = [self.hlsWriter.directoryPath stringByAppendingPathComponent:thumbnailPath];
...
- (void) startRecordingwithPFObject:(PFObject *)eventObject withTitle:(NSString *)titleText {
...
//    NSMutableString *firebaseURL = [[NSMutableString alloc] initWithString:@"https://godseye.firebaseio.com/feeds/"];
    ...
    if(eventObject == nil) {
        newEvent = [PFObject objectWithClassName:@"potentialEvents"];
...
//    self.feedRef = [[Firebase alloc] initWithUrl:firebaseURL];
//    self.feedRef = [self.feedRef childByAutoId];
    
//    self.feedRef = Firebase(url: "https://godseye.firebaseio.com/feeds/\(eventKey)")
//    self.feedRef = self.feedRef.childByAutoId()
...
//    [values setValue:@"Anonymous" forKey:@"author"];
//    [values setValue:@"live" forKey:@"status"];
//    [values setValue:[ forKey:@"image"];
//    [values setValue:@"Test" forKey:@"name"];
//    [values setValue:0 forKey:@"views"];
//    [values setValue: forKey:@"user"];
//    [self.feedRef setValue:@{
//                             @"author" : @"Anonymous",
//                             @"status": @"live",
//                             @"image": [NSString stringWithFormat:@"https://s3-us-west-1.amazonaws.com/crowdseye/%@/thumb.jpg", [self.feedRef key]],
//                             @"name" : titleText,
//                             @"views" : @0,
//                             @"user" : [[self.feedRef authData] uid],
//                             @"latitude" : @41,
//                             @"longitude" : @41
//                             }];
    ...
    endpoint.streamID = [self.feedObject objectId];
...
//        if ([endpointResponse isKindOfClass:[KFS3Stream class]]) {
    ...
    KFS3Stream *s3Endpoint = endpoint;
...
//    [self.locationManager stopUpdatingLocation];
    ...
    [self.feedObject setObject:@"vod" forKey:@"status"];
    [self.feedObject saveInBackground];
    
//    [self.feedRef updateChildValues:@{
//                                      @"status": @"vod"
//                                      }];
